...
static LineSmoother *touchesDelegate = NULL;

+ (LineSmoother *)instance
{
    @synchronized(self)
    {
        if (touchesDelegate == NULL)
            touchesDelegate = [[self alloc] init];
    }
    
    return touchesDelegate;
}

- (NSMutableArray*) smoothLine:(NSMutableArray*) points {
    NSArray *generalizedPoints = [self douglasPeucker:[NSArray arrayWithArray:points] epsilon:2];
    return [self catmullRomSpline:generalizedPoints segments:3];
}

//Algorithm to remove some vertices in an array representing a path
...
- (NSMutableArray *)catmullRomSpline:(NSArray *)points segments:(int)segments {
    int count = [points count];
    if(count < 4) {
        return [NSMutableArray arrayWithArray:points];
    }
